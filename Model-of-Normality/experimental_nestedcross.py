...
# Filter the multimodal data and labels using the balanced indices
...
# Outer CV: Stratified for evaluating the model's generalization
...
# Outer loop
for train_idx, test_idx in outer_cv.split(normalized_multimodal, labels):
    X_train, X_test = train_multimodal[train_idx], train_multimodal[test_idx]
    ...
    for params in ParameterGrid(param_grid):
        ...
        model = DepressionPredictor1()  # Re-instantiate the model
        for layer in model.classifier:
            if isinstance(layer, torch.nn.Dropout):
                layer.p = params['dropout_rate']  # Update dropout rate dynamically

        # Create DataLoader for inner CV training and validation
        ...
        for inner_train_idx, inner_val_idx in skf_inner.split(X_train, y_train):
            ...
            train_dataset = DepressionDatasetCross(torch.tensor(X_inner_train, dtype=torch.float32), torch.tensor(y_inner_train, dtype=torch.long))
            val_dataset = DepressionDatasetCross(torch.tensor(X_inner_val, dtype=torch.float32), torch.tensor(y_inner_val, dtype=torch.long))

            train_loader = DataLoader(train_dataset, batch_size=params['batch_size'], shuffle=True)
            val_loader = DataLoader(val_dataset, batch_size=params['batch_size'], shuffle=False)

            # Train and validate the model
            val_loss, val_accuracy = train_model(model, train_loader, val_loader, params)
            inner_val_scores.append(val_accuracy)

        avg_inner_val_accuracy = sum(inner_val_scores) / len(inner_val_scores)

        # Select the best hyperparameters
        if avg_inner_val_accuracy > best_accuracy:
            best_accuracy = avg_inner_val_accuracy
            best_params = params
