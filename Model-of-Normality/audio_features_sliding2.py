...
log_mel_dir = '../../../tudelft.net/staff-umbrella/EleniSalient/log_mel_sliding/'
# List all the files in the directory
log_mel_files = [f for f in os.listdir(log_mel_dir) if f.startswith('log_mel_') and f.endswith('.npy')]

# Sort the files if necessary (depends on how they are named, e.g., log_mel_1, log_mel_2, etc.)
log_mel_files.sort(key=lambda x: int(x.split('_')[-1].split('.')[0]))

# Initialize an empty list to store the arrays
log_mel_segments = []

# Load and append each array to the list
for file in log_mel_files:
    file_path = os.path.join(log_mel_dir, file)
    log_mel_segments.append(np.load(file_path))

# Combine all arrays into one
log_mel_seg = np.concatenate(log_mel_segments, axis=0)

print(f"Combined log-mel shape: {log_mel_seg.shape}")
