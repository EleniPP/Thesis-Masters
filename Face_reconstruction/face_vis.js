...
function renderFaceData(faceData)
{
	...
	console.log(`Current frame index: ${faceDataListIndex}`);
	console.log(`Frame timestamp: ${faceDataList[faceDataListIndex]?.timestamp} (min:sec)`);
	console.log(`Audio current time: ${audioPlayer.currentTime} seconds`);
	...
	for (var pointIndex = 0; pointIndex < points.length; pointIndex += 1)
	{
		...
		context.arc(normalisedX, normalisedY, 2 + 2*point.c, 0, 2*Math.PI);
...
function syncFaceToAudio() {
    const currentTime = audioPlayer.currentTime / 60; // Convert seconds to minutes
    ...
    console.log(`Audio time (min:sec): ${currentTime.toFixed(5)} minutes`);
    console.log(`Current frame index: ${faceDataListIndex}`);
    console.log(`Frame timestamp: ${faceDataList[faceDataListIndex]?.timestamp} minutes`);
}
